...
function row(label: string, value: string): string {
  return `<tr><td style="padding:6px 12px;color:#6b6b6b;">${label}</td><td style="padding:6px 12px;font-weight:600;">${escHtml(value)}</td></tr>`;
}
...
export function renderNewBookingAdminEmail(
  ...
  <div style="font-family:system-ui,sans-serif;max-width:560px;margin:auto;">
    ...
    <table style="width:100%;border-collapse:collapse;background:#fdf7f4;">
      ...
      ${row('Price', price)}
      ${ctx.customerNotes ? row('Notes', ctx.customerNotes) : ''}
    </table>
    ${buttons}
...
export function renderCustomerConfirmationEmail(ctx: BookingEmailContext): { subject: string; html: string; text: string } {
  ...
  const safeName = escHtml(ctx.customerName);
  const greeting = isZh ? `您好 ${safeName}` : `Hi ${safeName}`;
  const intro = isZh

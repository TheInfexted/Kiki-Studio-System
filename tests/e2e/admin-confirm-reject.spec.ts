...
test('confirm a pending booking from the dashboard', async ({ page }) => {
  ...
  const row = page.locator('details').filter({ hasText: 'E2E Customer 101' });
  await row.click();
  await row.getByRole('button', { name: /^Confirm$/ }).click();

  await expect(row).toContainText(/confirmed/i, { timeout: 5000 });
...
test('reject with reason emails the customer the reason', async ({ page }) => {
  ...
  await signIn(page);
  const row = page.locator('details').filter({ hasText: 'E2E Customer 102' });
  await row.click();
  await row.getByRole('button', { name: /^Reject$/ }).click();
  await row.locator('textarea').fill('Fully booked that weekend');
  await row.getByRole('button', { name: /confirm rejection/i }).click();

  await expect(row).toContainText(/rejected/i, { timeout: 5000 });
